...
class WorldGenerator:
    ...
    async def generate_detailed_world(self, basic_world: WorldSetting,
        ...
        focus_areas = focus_areas or ["政治", "经济", "文化", "历史"]

        for area in focus_areas:
            prompt = self.prompt_manager.get_prompt(
                "worldbuilding",
                f"detailed_{area.lower()}",
            ...
            # 更新世界设定
            updated_data = await self._parse_detail_response(response.content, area)
            basic_world = self._merge_world_data(basic_world, updated_data)
    ...
    async def _parse_world_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 尝试解析JSON
            try:
                logger.debug(f"parse_llm_response==>{structure_response.content}")
                # 尝试提取JSON部分
...
class WorldBuilderTool(AsyncTool):
    ...
    async def execute(self, parameters: Dict[str, Any],
        ...
        if detail_level == "detailed":
            # 生成详细世界
            detailed_world = await self.generator.generate_detailed_world(basic_world, focus_areas)
